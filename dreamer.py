...
class Dreamer(nn.Module):
    ...
    def _policy(self, obs, state, training):
        ...
        state = (latent, action)
        return policy_output, state, tools.TwoHotDistSymlog(logits=value_params).mode().reshape(-1).item()
...
def main(config, defaults):
    ...
    print("net layer 1, 1", torch.linalg.matrix_norm(agent._wm.heads["reward"].layers[1].fn.net[0].weight))
    print("net2 layer 1, 1", torch.linalg.matrix_norm(agent._wm.heads["reward"].layers[1].fn.net2[0].weight))
    print("net layer 2, 1", torch.linalg.matrix_norm(agent._wm.heads["reward"].layers[3].fn.net[0].weight))
    print("net2 layer 2, 2", torch.linalg.matrix_norm(agent._wm.heads["reward"].layers[3].fn.net2[0].weight))
    print("task behavior")
    print("stoch layer 1", torch.linalg.matrix_norm(agent._task_behavior.a2c.stoch_layer[0].weight))
    print("stoch layer 2", torch.linalg.matrix_norm(agent._task_behavior.a2c.stoch_layer[2].weight))
    print("deter layer 1", torch.linalg.matrix_norm(agent._task_behavior.a2c.deter_layer[0].weight))
    print("deter layer 2", torch.linalg.matrix_norm(agent._task_behavior.a2c.deter_layer[2].weight))
    print("net layer 1, 1", torch.linalg.matrix_norm(agent._task_behavior.a2c.layers[1].fn.net[0].weight))
    print("net2 layer 1, 1", torch.linalg.matrix_norm(agent._task_behavior.a2c.layers[1].fn.net2[0].weight))
    print("net layer 2, 1", torch.linalg.matrix_norm(agent._task_behavior.a2c.layers[3].fn.net[0].weight))
    print("net2 layer 2, 2", torch.linalg.matrix_norm(agent._task_behavior.a2c.layers[3].fn.net2[0].weight))
    exit(1)
    state = None

...
class Dreamer(nn.Module):
    ...
    def __call__(self, obs, reset, state=None, reward=None, training=True):
        ...
        if training:
            ...
            if self._should_log(step):
                ...
                print("right before", metrics_dict)
    ...
    def _policy(self, obs, state, training):
        ...
        target_embedding = self._wm.embedding(target_array)
        feat = self._wm.dynamics.get_feat(latent)
        if not training:
            actor = self._task_behavior.actor(feat, target_embedding)
            action = actor.mode()
        elif self._should_expl(self._step):
            actor = self._expl_behavior.actor(feat, target_embedding)
            action = actor.sample()
        else:
            actor = self._task_behavior.actor(feat, target_embedding)
            action = actor.sample()
    ...
    def _train(self, data):
        ...
        print("_train merge", self._metrics)

    ...
    def subsample(self, ret, markers, dist, batch_size, tuples, episode_sizes, aggregate_sizes):
        ...
        for i in range(batch_size):
            ...
            while size < self.batch_length:
                ...
                start_frame = self.random.randint(slices_in_episode[index][0], slices_in_episode[index][1])
                ...
                while index < len(slices_in_episode) and size < self.batch_length and curr_target_frame < frame_counts[curr_target]:
                    ...
                    ret = {
                        ...
                        for k, v in episode.items() if k not in ["augmented", "objects", "stats", "inventory"]
                    }
                    if markers is None:
    ...
    while True:
        ...
        while size < length:
            ...
            if not ret:
                ...
                ret = {
                    k: v[index : min(index + length, total)] for k, v in episode.items() if k not in ["augmented", "objects", "stats", "inventory"]
                }
            else:
...
class Normal:
    def __init__(self, mean, shape=1, device="cuda"):
        std = torch.ones_like(mean).to(device)
        self.dist = ContDist(

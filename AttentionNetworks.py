...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        print("linear", x.abs().mean().item(), q.abs().mean().item())
        print("linear weights", self.net[0].abs().mean().item(), self.net[2].abs().mean().item(),
              self.net2[0].abs().mean().item(), self.net2[2].abs().mean().item())
        return x, q
